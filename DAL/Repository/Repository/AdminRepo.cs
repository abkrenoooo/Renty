...
namespace DAL.Repository.Repository
{
    public class AdminRepo : IAdminRepo
    {
        ...
        #region Create
        public async Task<Response<ApplicationUser>> CreateAdminUserAsync(AdminUserVM model)
        {
            try
            {
        ...
        public async Task<Response<ApplicationUser>> EditAdminUserAsync(AdminUserVM model)
        {
            try
            {
                if (await _userManager.FindByEmailAsync(model.Email) is not null)
                ...
                if (user == null)
                {
                    ...
                    {
                        Success = false,
                        Message = "Can't Found Admin",
                        status_code = "404",
                    };
                }
                var editAdmin = user.ToApplicationUserToEdit(model.ToApplicationUser().Result).Result;
        ...
        public async Task<Response<ApplicationUser>> GetAdminUserByIdAsync(string AdminUserId)
        {
            try
            {
                var admin = await _userManager.FindByIdAsync(AdminUserId);
        ...
        public async Task<Response<ApplicationUser>> GetAllAdminAsync(int paggingNumber)
        {
            try
            {
                ...
                var AllAdmin = admins.Skip((paggingNumber - 1) * 10).Take(10).ToList();
                return new Response<ApplicationUser>
                {
                    Success = true,
                    Message = "All Admins",
